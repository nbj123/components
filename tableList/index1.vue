  ...
  <div>
    <el-card class="box-card">
      ...
      <div class="box-card-body">
        <div class="el-tabs-table el-tabs-table-padding">
          ...
          >
            <el-table-column v-if="showIndex" fixed="left" type="index"></el-table-column>
            <el-table-column
              width="55"
              v-if="!!selectlistRow && !tableList[0].fixed"
            ...
            >
              <div v-if="item.id || item.id == 0">
                <!-- 无插槽，有排序 -->
                <el-table-column
                  :key="index"
                  v-if="!item.solt && item.sortable && titleLists[item.id]"
                  sortable="custom"
                  :sort-orders="['ascending', 'descending']"
                  :prop="item.prop"
                  :label="item.name ? $t(`main.${item.name}`) : item.label"
                ></el-table-column>
                <!-- 无插槽，无排序 -->
                <el-table-column
                  v-else-if="!item.solt && !item.sortable && titleLists[item.id]"
                  :prop="item.prop"
                  :label="item.name ? $t(`main.${item.name}`) : item.label"
                ></el-table-column>
                <!-- 有插槽 -->
              ...
              <div v-else>
                ...
                <!-- 无插槽，无排序 -->
                <el-table-column
                  v-else-if="!item.solt && !item.sortable"
                  :prop="item.prop"
                  :label="item.name ? $t(`main.${item.name}`) : item.label"
                ></el-table-column>
                <!-- 有插槽 -->
    ...
    <div id="menu" v-if="menusList.length > 0">
      <div
        ...
        :key="index"
        @click.stop="item.eventName(rightClickData)"
      >
        <div>{{ item.name }}</div>
      </div>
    </div>
